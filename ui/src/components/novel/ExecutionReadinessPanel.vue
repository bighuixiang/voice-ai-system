<template>
  <section class="execution-readiness-panel" data-testid="execution-readiness-panel" aria-labelledby="execution-readiness-title">
    ...
    <div v-if="proof" class="proof-summary"><strong>证明 {{ proof.status }}</strong><span>是否可执行：{{ String(proof.executionReady) }} · 版本：{{ proof.versionId }}</span><ul><li v-for="check in proof.checks" :key="check.checkId">{{ check.checkId }}: {{ check.status }} — {{ check.detail }}</li></ul></div>
