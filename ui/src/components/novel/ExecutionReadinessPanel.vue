<template>
  <section class="execution-readiness-panel" data-testid="execution-readiness-panel" aria-labelledby="execution-readiness-title">
    <header class="panel-header"><div><p class="eyebrow">执行门禁</p><h2 id="execution-readiness-title">执行就绪证明</h2><p>证明、版本指针和章节窗口必须同时通过；大纲提交本身不等于可执行。</p></div><button type="button" aria-label="刷新执行证明" :disabled="loading" @click="emit('refresh-proof')">{{ loading ? "刷新中…" : "刷新证明" }}</button></header>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div v-if="proof" class="proof-summary"><strong>证明 {{ statusLabel(proof.status) }}</strong><span>是否可执行：{{ booleanLabel(proof.executionReady) }} · 版本：{{ proof.versionId }}</span><ul><li v-for="check in proof.checks" :key="check.checkId">{{ checkIdLabel(check.checkId) }}：{{ statusLabel(check.status) }} — {{ check.detail }}</li></ul></div>
    ...
    <ul v-if="readiness" class="readiness-checks"><li v-for="check in readiness.checks" :key="check.checkId">{{ checkIdLabel(check.checkId) }}：{{ statusLabel(check.status) }} — {{ check.detail }}</li></ul>
    <div class="work-items"><strong>执行工作项</strong><p v-if="!workItems.length">暂无已排队工作项。</p><ul v-else><li v-for="item in workItems" :key="item.workItemId">{{ item.chapterId }} · {{ statusLabel(item.status) }}<span v-if="item.blockedReason"> · {{ item.blockedReason }}</span></li></ul></div>
  </section>
</template>

<script setup lang="ts">
import type { ExecutionReadinessDecision, ExecutionReadyProof, ExecutionWorkItem } from "@/types/novel";
import { booleanLabel, checkIdLabel, statusLabel } from "@/utils/novelLabels";
