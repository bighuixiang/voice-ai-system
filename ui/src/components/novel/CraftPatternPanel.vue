<template>
  <section class="craft-pattern-panel">
    <header><strong>工艺模式审阅</strong><small>候选与正式设定隔离</small></header>
    ...
    <article v-for="item in patterns" :key="item.patternId" class="pattern-card">
      <div class="pattern-title"><strong>{{ item.name }}</strong><span>{{ statusLabel(item.lifecycle) }}</span></div>
      ...
      <small>不会自动写入正式设定；必须经过独立实验和作者决定。</small>
      <button v-if="item.lifecycle === 'probation' && judgedExperiments(item).length" type="button" :data-testid="`validate-pattern-${item.patternId}`" @click="emit('validate', item, judgedExperiments(item)[0])">验证为已验证</button>
      <button v-if="item.lifecycle === 'approved'" type="button" :data-testid="`promote-pattern-${item.patternId}`" @click="emit('promote', item)">申请试验</button>
    </article>
