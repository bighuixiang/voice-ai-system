...
describe("runtime autopilot infrastructure", () => {
  ...
  it("governed-runtime-chapter-execution-plan keeps governed runtime prose outside canon until adoption", async () => {
    ...
    const root = projectRoot(project.slug);
    const chapter = project.chapters[0];
    const versionBase = { schemaVersion: "outline-version.v1", versionId: "outline-version-governed", projectSlug: project.slug, version: 1, outlineId: "outline-governed", outlineFingerprint: "outline-fp", selectedChapterIds: [chapter.id], strongFreezeCount: 1, structureVersionFingerprint: "outline-fp", changeLevel: "L0", adoptionAuthority: "author", adoptionProofFingerprint: "adoption-proof-1", status: "active", canonWritten: true, createdAt: new Date().toISOString() };
