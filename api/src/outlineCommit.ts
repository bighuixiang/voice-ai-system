...
export function assertOutlineVersionIntegrity(version: OutlineVersion, expectedId?: string): OutlineVersion {
  ...
  const valid = version.schemaVersion === "outline-version.v1" && (!expectedId || version.outlineId === expectedId || version.versionId === expectedId || version.versionId === `outline-version-${expectedId}`) && Boolean(version.versionId?.trim() && version.projectSlug?.trim() && version.outlineId?.trim() && version.outlineFingerprint?.trim() && version.structureVersionFingerprint?.trim() && version.adoptionAuthority?.trim() && version.adoptionProofFingerprint?.trim()) && (version.comparisonFingerprint === undefined || /^[a-f0-9]{64}$/i.test(version.comparisonFingerprint)) && Number.isInteger(version.version) && version.version > 0 && Array.isArray(version.selectedChapterIds) && version.selectedChapterIds.length > 0 && version.selectedChapterIds.every((id) => typeof id === "string" && id.trim()) && Number.isInteger(version.strongFreezeCount) && version.strongFreezeCount >= 0 && version.strongFreezeCount <= version.selectedChapterIds.length && ["L0", "L1", "L2"].includes(version.changeLevel) && version.status === "active" && version.canonWritten === true && typeof version.createdAt === "string" && Number.isFinite(Date.parse(version.createdAt)) && /^[a-f0-9]{64}$/i.test(version.fingerprint) && fingerprint(base) === version.fingerprint;
