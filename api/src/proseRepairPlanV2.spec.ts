...
describe("evidence-driven prose repair plan", () => {
  ...
  it("requires unique finite issue values, nonblank evidence, and explicit authority/risk", () => {
    expect(() => createEvidenceRepairPlan({ ...base, issues: [{ ...base.issues[0], issueId: "" }] })).toThrow("REPAIR_ISSUE_ID_REQUIRED");
    expect(() => createEvidenceRepairPlan({ ...base, issues: [{ ...base.issues[0], value: Number.NaN }] })).toThrow("REPAIR_ISSUE_VALUE_INVALID");
    expect(() => createEvidenceRepairPlan({ ...base, issues: [{ ...base.issues[0], evidenceRefs: [" "] }] })).toThrow("REPAIR_ISSUE_EVIDENCE_REQUIRED");
    expect(() => createEvidenceRepairPlan({ ...base, issues: [{ ...base.issues[0], issueId: "i2" }, ...base.issues.slice(1)] })).toThrow("REPAIR_ISSUE_DUPLICATE");
    expect(() => createEvidenceRepairPlan({ ...base, collateralRisks: [] })).toThrow("REPAIR_RISK_REQUIRED");
    const plan = createEvidenceRepairPlan(base);
    expect(plan.modificationAuthority).toBe("author");
    expect(plan.collateralRisks).toEqual(base.collateralRisks);
  });
});
