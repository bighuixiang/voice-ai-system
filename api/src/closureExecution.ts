...
export function createClosureSchedule(input: { obligations: ReadonlyArray<{ id: string; dependencies: readonly string[]; window: string }>; availableWindows: readonly string[] }): ClosureSchedule {
  const ids = new Set(input.obligations.map((item) => item.id)); const risks: string[] = [];
  for (const item of input.obligations) for (const dependency of item.dependencies) if (!ids.has(dependency)) risks.push(`missing-dependency:${item.id}:${dependency}`);
  const graph = new Map(input.obligations.map((item) => [item.id, item.dependencies.filter((dependency) => ids.has(dependency))])); const visiting = new Set<string>(); const visited = new Set<string>(); const hasCycle = (id: string): boolean => { if (visiting.has(id)) return true; if (visited.has(id)) return false; visiting.add(id); for (const dependency of graph.get(id) ?? []) if (hasCycle(dependency)) return true; visiting.delete(id); visited.add(id); return false; }; if ([...ids].some((id) => hasCycle(id))) risks.push("dependency-cycle"); for (const item of input.obligations) if (!input.availableWindows.includes(item.window)) risks.push(`unreachable:${item.id}`); const assignments: Record<string, string> = {}; for (const item of input.obligations) assignments[item.id] = item.window; const base = { schemaVersion: "closure-schedule.v1" as const, status: risks.length ? "blocked" as const : "ready" as const, risks: [...new Set(risks)], assignments };
  return { ...base, fingerprint: hash(base) };
}
export function reservePayoffCapacity(input: { window: string; capacity: number; reservations: ReadonlyArray<{ obligationId: string; load: number }> }): PayoffReservation {
...
export function detectExposureRisk(input: { obligationId: string; reminders: ReadonlyArray<{ hypothesis: string; chapter: number }>; characterAttentionCount: number; competingHypotheses: number }): ExposureRisk {
...
export function propagateClosureOutcome(input: { payoffId: string; changes: ReadonlyArray<{ domain: string; before: string; after: string }>; downstreamRefs: readonly string[] }): ClosureOutcome {
