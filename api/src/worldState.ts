...
export async function readWorldStateSnapshot(root: string, snapshotId: string): Promise<WorldStateSnapshot | null> { return readJson<WorldStateSnapshot>(snapshotPath(root, snapshotId)); }
export async function listWorldStateSnapshots(root: string, projectSlug: string, region?: string): Promise<WorldStateSnapshot[]> {
  ...
  const records = await Promise.all(names.filter((name) => name.endsWith(".json")).map((name) => readJson<WorldStateSnapshot>(path.join(directory, name))));
...
export async function recordWorldStateSnapshot(input: Omit<WorldStateSnapshot, "schemaVersion" | "snapshotId" | "createdAt" | "fingerprint">): Promise<WorldStateSnapshot> {
