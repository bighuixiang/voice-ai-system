...
async function fixture(root: string, includeWorld = true) {
  ...
  const proposalBase = { schemaVersion: "story-contract-adoption-proposal.v1" as const, proposalId: "proposal-1", candidateId: "candidate-1", candidateFingerprint: candidate.fingerprint, projectSlug: "demo", status: "committed" as const, fieldDecisions: fields.map((field) => ({ fieldId: field.fieldId, status: "accept" as const })), acceptedFields: fields, unresolvedFieldIds: [] as string[], reviewId: "review-1", canonWritten: true as const, createdAt: new Date().toISOString(), committedMutationId: "mutation-1", committedAt: new Date().toISOString() };
  await fs.writeFile(path.join(root, "sessions", "story-contract-adoption-proposal.json"), JSON.stringify({ ...proposalBase, fingerprint: crypto.createHash("sha256").update(JSON.stringify(proposalBase)).digest("hex") }), "utf8");
  const reviewBase = { schemaVersion: "understanding-review.v1", reviewId: "review-1", projectSlug: "demo", snapshotId: "snapshot-1", snapshotFingerprint: candidate.sourceFingerprint, calibrationVersion: "understanding-calibration.v1", reviewer: { kind: "independent-deterministic", id: "reviewer" }, status: "passed", checks: ["source-fingerprint", "evidence-spans", "branch-separation", "question-gate", "canon-isolation"].map((checkId) => ({ checkId, status: "passed", detail: "fixture" })), canonWritten: false, createdAt: new Date().toISOString() };
...
describe("story contract readiness proof", () => {
  ...
  it("reports partial fields and blocks missing authorization or stale projections", async () => {
    ...
    const committed = JSON.parse(await fs.readFile(path.join(root, "sessions", "story-contract-adoption-proposal.json"), "utf8")) as Record<string, unknown>;
    const { fingerprint: _fingerprint, committedMutationId: _committedMutationId, committedAt: _committedAt, ...authorizationBase } = committed;
    const authorizationProposal = { ...authorizationBase, status: "ready_for_authorization", canonWritten: false };
    await fs.writeFile(path.join(root, "sessions", "story-contract-adoption-proposal.json"), JSON.stringify({ ...authorizationProposal, fingerprint: crypto.createHash("sha256").update(JSON.stringify(authorizationProposal)).digest("hex") }), "utf8");
