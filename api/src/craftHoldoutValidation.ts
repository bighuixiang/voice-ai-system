...
export function validateCraftHoldout(input: CraftHoldoutInput): CraftHoldoutResult {
  ...
  if (!input.sourceRefs.length) throw new Error("CRAFT_HOLDOUT_SOURCE_REQUIRED");
  ...
  const status = issues.some((issue) => issue === "HOLDOUT_EXTRACTION_OVERLAP" || issue === "HOLDOUT_LABEL_LEAK" || issue === "HOLDOUT_HARD_GUARD_FAILED")
    ? "blocked"
