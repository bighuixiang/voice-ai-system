...
export interface ExecutionWorkItem {
  ...
  blockedReason?: "PROOF_NOT_FOUND" | "PROOF_BLOCKED" | "VERSION_POINTER_STALE" | "CHAPTER_OUTSIDE_WINDOW" | "CONTEXT_MANIFEST_REQUIRED" | "CONTEXT_MANIFEST_STALE";
  createdAt: string;
  claimedAt?: string;
  heartbeatAt?: string;
  finishedAt?: string;
...
export async function readExecutionWorkItem(root: string, itemId: string): Promise<ExecutionWorkItem | null> {
  try { return JSON.parse(await fs.readFile(itemPath(root, itemId), "utf8")) as ExecutionWorkItem; }
...
export async function listExecutionWorkItems(root: string): Promise<ExecutionWorkItem[]> {
  ...
  for (const name of names.filter((entry) => entry.endsWith(".json"))) {
    try { items.push(JSON.parse(await fs.readFile(path.join(directory, name), "utf8")) as ExecutionWorkItem); } catch { /* ignore transient or corrupt files; recovery audit handles them */ }
  }
  return items.sort((left, right) => left.createdAt.localeCompare(right.createdAt) || left.workItemId.localeCompare(right.workItemId));
}

export async function enqueueExecutionWorkItem(root: string, projectSlug: string, chapterId: string, idempotencyKey: string): Promise<ExecutionWorkItem> {
...
export async function claimExecutionWorkItem(root: string, itemId: string, runId: string): Promise<ExecutionWorkItem> {
  return withItemLock(root, itemId, async () => {
    ...
    const nextBase = { ...item, status: "running" as const, runId, claimedAt: now, heartbeatAt: now, createdAt: item.createdAt };
...
export async function heartbeatExecutionWorkItem(root: string, itemId: string, runId: string): Promise<ExecutionWorkItem> {
  return withItemLock(root, itemId, async () => {
    const item = await readExecutionWorkItem(root, itemId);
    if (!item) throw new Error("EXECUTION_WORK_ITEM_NOT_FOUND");
    if (item.status !== "running" || item.runId !== runId) throw new Error("EXECUTION_WORK_ITEM_RUN_FENCE");
    const nextBase = { ...item, heartbeatAt: new Date().toISOString(), createdAt: item.createdAt };
    const { fingerprint: _old, ...nextWithoutFingerprint } = nextBase;
    const next: ExecutionWorkItem = { ...nextWithoutFingerprint, fingerprint: fingerprint(nextWithoutFingerprint) };
    await writeJson(itemPath(root, itemId), next);
    return next;
...
export async function finishExecutionWorkItem(root: string, itemId: string, input: { status: "completed" | "failed"; error?: string; runId?: string }): Promise<ExecutionWorkItem> {
  return withItemLock(root, itemId, async () => {
    const item = await readExecutionWorkItem(root, itemId);
    if (!item) throw new Error("EXECUTION_WORK_ITEM_NOT_FOUND");
