...
import { createRevisionChangeSet, readRevisionChangeSet } from "./revisionChangeSet.js";
...
describe("semantic revision change set", () => {
  ...
  it("fails closed when a change set is tampered", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "revision-changeset-tampered-"));
    const intent = await createRevisionIntent(root, { projectSlug: "demo", authorText: "Change.", type: "style_edit", maturity: "settled", scope: { chapterIds: ["chapter-008"] }, requestedChanges: ["local"], protectedItems: [], mode: "branch_candidate", actor: "author" });
    const changeSet = await createRevisionChangeSet(root, intent.intentId, intent.fingerprint, [{ kind: "update", targetKind: "text-span", targetId: "chapter-008.paragraph-5", chapterId: "chapter-008", rationale: "local" }]);
    const target = path.join(root, "sessions", "revisions", "changesets", `${changeSet.changeSetId}.json`);
    await fs.writeFile(target, JSON.stringify({ ...changeSet, status: "committed" }), "utf8");
    await expect(readRevisionChangeSet(root, changeSet.changeSetId)).rejects.toThrow("REVISION_CHANGESET_INTEGRITY_FAILED");
  });
});
