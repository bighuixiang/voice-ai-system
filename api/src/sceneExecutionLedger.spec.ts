...
describe("scene execution ledger", () => {
  ...
  it("fails closed when persisted execution evidence is tampered", async () => {
    const r = await root();
    const ledger = await createSceneExecutionLedger({ root: r, sceneCard: await card(r), obligationIds: ["obl-1"] });
    const target = path.join(r, "sessions", "scene-execution-ledgers", `${ledger.ledgerId}.json`);
    ...
    await expect(readSceneExecutionLedger(r, ledger.ledgerId)).rejects.toThrow("SCENE_LEDGER_INTEGRITY_FAILED");
  });
  it("blocks evidence after the frozen scene card fingerprint drifts", async () => {
    const r = await root();
    const scene = await card(r);
    const ledger = await createSceneExecutionLedger({ root: r, sceneCard: scene, obligationIds: [] });
    const target = path.join(r, "sessions", "scene-cards", `${scene.sceneId}.json`);
    const persisted = JSON.parse(await fs.readFile(target, "utf8")) as Record<string, unknown>;
    const { fingerprint: _fingerprint, ...base } = persisted;
    const changed = { ...base, roleGoal: "take the key" };
    const fingerprint = crypto.createHash("sha256").update(JSON.stringify(changed)).digest("hex");
    await fs.writeFile(target, JSON.stringify({ ...changed, fingerprint }), "utf8");
    await expect(appendSceneExecutionEvidence(r, ledger.ledgerId, { kind: "choice", value: "chooses the locked door", evidenceRefs: ["prose://scene-1#choice"] })).rejects.toThrow("SCENE_LEDGER_SCENE_CARD_STALE");
  });
});
