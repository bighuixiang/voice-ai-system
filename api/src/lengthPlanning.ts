...
export function assertLengthContractIntegrity(contract: LengthContract): LengthContract {
  ...
  const dimensions = contract && typeof contract.dimensions === "object" && contract.dimensions !== null ? contract.dimensions : undefined;
  const requiredDimensions = dimensions ? [dimensions.totalWords, dimensions.totalChapters, dimensions.totalVolumes, dimensions.chapterWords] : [];
  const validTimestamp = (value: unknown): value is string => typeof value === "string" && value.trim() !== "" && Number.isFinite(Date.parse(value));
  const valid = Boolean(contract && contract.schemaVersion === "length-contract.v1" && typeof contract.projectSlug === "string" && contract.projectSlug.trim() && requiredDimensions.length === 4 && requiredDimensions.every(validDimension) && Number.isFinite(contract.pauseThresholdRatio) && contract.pauseThresholdRatio >= 0 && contract.pauseThresholdRatio <= 1 && Array.isArray(contract.hardLocks) && contract.hardLocks.every((key) => ["totalWords", "totalChapters", "totalVolumes", "chapterWords"].includes(key)) && contract.hardLocks.every((key) => dimensions?.[key as keyof typeof dimensions]?.mode === "hard") && contract.source === "author" && contract.effectiveScope === "project" && Array.isArray(contract.revisionLineage) && contract.revisionLineage.every((value) => typeof value === "string" && value.trim()) && validTimestamp(contract.createdAt) && /^[a-f0-9]{64}$/i.test(contract.fingerprint) && hash(base) === contract.fingerprint);
