...
describe("release preflight", () => {
  ...
  it("is ready only when all current immutable release inputs verify", async () => {
    ...
    const artifactEntry = { format: "markdown", relativePath: "sessions/publication-editions/edition-1.md", mime: "text/markdown; charset=utf-8", rendererVersion: "publication-renderer.v1", sha256: crypto.createHash("sha256").update(body).digest("hex"), size: body.byteLength };
    const artifactBase = { schemaVersion: "publication-artifact-set.v1", artifactSetId: "a", editionId: "edition-1", projectSlug: "demo", manifestFingerprint, treeFingerprint, status: "validated", artifacts: [artifactEntry], createdAt: "now" };
    ...
    const proofBase = { schemaVersion: "delivery-proof.v1", proofId: "p", editionId: "edition-1", projectSlug: "demo", manifestFingerprint: artifact.manifestFingerprint, treeFingerprint: artifact.treeFingerprint, artifactSetFingerprint: artifact.fingerprint, artifactHashes: [{ format: artifactEntry.format, relativePath: artifactEntry.relativePath, sha256: artifactEntry.sha256, size: artifactEntry.size }], approvalId: "author", approverKind: "author", status: "issued", issuedAt: "2026-08-04T00:00:00.000Z" };
    ...
    await fs.writeFile(path.join(dir, "edition-1.md"), "# Demo\nAUTHOR_TRUTH\n", "utf8");
    const leaked = await buildReleasePreflight(root, "edition-1");
    expect(leaked.status).toBe("blocked");
    expect(leaked.findings.map((finding) => finding.code)).toContain("artifact-reader-leak");
  });
});
