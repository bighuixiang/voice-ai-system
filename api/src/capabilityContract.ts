...
export function assertCapabilityContractIntegrity(contract: CapabilityContract, expectedId?: string): CapabilityContract { const { fingerprint: _fingerprint, ...base } = contract; const valid = contract.schemaVersion === "capability-contract.v1" && (!expectedId || contract.capabilityId === expectedId) && Boolean(contract.capabilityId?.trim() && contract.projectSlug?.trim() && contract.holderId?.trim() && contract.name?.trim() && contract.canDo?.trim() && contract.duration?.trim() && contract.cooldown?.trim() && contract.precision?.trim()) && [contract.sourceRefs, contract.cannotDo, contract.prerequisites, contract.inputs, contract.consumption, contract.scope, contract.counters, contract.progressionPath, contract.evidenceRefs, contract.progressionEventIds].every((values) => Array.isArray(values) && values.every((value) => typeof value === "string" && value.trim())) && contract.permanent === false && ["candidate", "confirmed", "retired"].includes(contract.status) && /^[a-f0-9]{64}$/i.test(contract.fingerprint) && hash(base) === contract.fingerprint; if (!valid) throw new Error("CAPABILITY_CONTRACT_INTEGRITY_FAILED"); return contract; }
export function assertProgressionEventIntegrity(event: ProgressionEvent, expectedId?: string): ProgressionEvent { const { fingerprint: _fingerprint, ...base } = event; const valid = event.schemaVersion === "progression-event.v1" && (!expectedId || event.progressionId === expectedId) && Boolean(event.progressionId?.trim() && event.capabilityId?.trim() && event.trigger?.trim() && event.acquisition?.trim() && event.retained?.trim() && event.abandoned?.trim() && event.limitation?.trim() && event.newChoice?.trim()) && ["permanent", "temporary-boost", "borrowed-tool", "external-aid", "discovery", "exchange"].includes(event.acquisitionKind) && [event.proseRefs, event.sourceRefs].every((values) => Array.isArray(values) && values.length > 0 && values.every((value) => typeof value === "string" && value.trim())) && /^[a-f0-9]{64}$/i.test(event.fingerprint) && hash(base) === event.fingerprint; if (!valid) throw new Error("PROGRESSION_EVENT_INTEGRITY_FAILED"); return event; }
export async function readCapabilityContract(root: string, capabilityId: string): Promise<CapabilityContract | null> { const value = await readJson<CapabilityContract>(capabilityPath(root, capabilityId)); return value ? assertCapabilityContractIntegrity(value, capabilityId) : null; }
export async function listCapabilityContracts(root: string, projectSlug: string): Promise<CapabilityContract[]> { const directory = resolveInside(root, "sessions/capability-contracts"); let names: string[]; try { names = await fs.readdir(directory); } catch (error) { if (error instanceof Error && "code" in error && (error as { code?: string }).code === "ENOENT") return []; throw error; } const records = await Promise.all(names.filter((name) => name.endsWith(".json")).map(async (name) => { const record = await readJson<CapabilityContract>(path.join(directory, name)); return record ? assertCapabilityContractIntegrity(record, name.slice(0, -5)) : null; })); return records.filter((record): record is CapabilityContract => Boolean(record && record.projectSlug === projectSlug)).sort((a, b) => a.createdAt.localeCompare(b.createdAt)); }
...
export async function createProgressionEvent(input: { root: string; capabilityId: string; trigger: string; acquisition: string; acquisitionKind?: AcquisitionKind; retained: string; abandoned: string; limitation: string; newChoice: string; proseRefs: readonly string[]; sourceRefs: readonly string[] }): Promise<ProgressionEvent> {
  ...
  const existing = await readJson<ProgressionEvent>(progressionPath(input.root, progressionId)); if (existing) return assertProgressionEventIntegrity(existing, progressionId);
  ...
  const updatedBase = { ...capability, status: "confirmed" as const, progressionEventIds: [...capability.progressionEventIds, progressionId], updatedAt: new Date().toISOString() };
  const { fingerprint: _previousFingerprint, ...updatedWithoutFingerprint } = updatedBase;
  const updated: CapabilityContract = { ...updatedWithoutFingerprint, fingerprint: hash(updatedWithoutFingerprint) };
  await writeJson(capabilityPath(input.root, capability.capabilityId), updated);
  return event;
}
